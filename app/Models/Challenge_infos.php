...
class Challenge_infos extends Model
{
    ...
    public function challengeMilestoneComplete($id, $user_id, $milesDist, $category){ 
        ...
        if($category == 'individual'){
            ...
            $ch = DB::table('challenge_logs')->where([['participation_id', '=', $id]])->groupBy('created_at')->groupBy('challenge_logs.activity_id')->get();
            $ch = json_encode($ch);
            $ch = json_decode($ch, true);
        }
    ...
    public function challengeMilestoneCompleteBadgeAssignOnLogAdd($id, $user_id, $milesDist, $distance, $category){ 
    ...
    public function challengeComplete($id, $user_id, $milesDist){
        ...
        $challenge_infos = DB::table('challenge_infos')->where([['challenge_id', '=', $id], ['meta_name', '=', 'category']])->first();
        $challenge_infos = json_encode($challenge_infos);
        $challenge_infos = json_decode($challenge_infos, true);
        
        $category = '';
        if($challenge_infos){
            $category = $challenge_infos['meta_value'];
        }
    ...
    public function challengeMilestoneCompleteAtSpecificDateBadgeAssignOnLogAdd($participation_id, $user_id, $milesDist, $specificDateTime, $timezone, $distance, $category){
        ...
        if($category == 'individual'){
            ...
            $ch = DB::table('challenge_logs')->where([['participation_id', '=', $participation_id]])->whereBetween('startDateTime', array($start_date, $end_date))->groupBy('created_at')->groupBy('activity_id')->get();
  ...
  //echo '<pre>'; print_r($ch);
    ...
    public function challengeMilestoneCompleteAtSpecificDate($participation_id, $user_id, $milesDist, $specificDateTime, $timezone, $category){
        $start_date = Carbon::parse($specificDateTime)->startOfDay();
        $end_date = Carbon::parse($specificDateTime)->endOfDay();
    ...
    public function checkAndActivateChallenge($challenge_id, $distance, $user_id, $start_date_time){
        ...
        if($coveredDistance < $total_distance){
            ...
            if($category == 'individual'){
                ...
                /*$ucha1 = json_encode($ucha);
                $ucha1 = json_decode($ucha1, true);
                if($ucha1){
                    $userB = User_badges::find($ucha1['id']);
                                    $userB->delete();
                }*/
                $ucha->status = 1;
                $ucha->update();
                ...
                DB::table('user_challenges')->where('challenge_id', $challenge_id)->update(['status' => 1]);
                //echo $challenge_id.$category; die();
        ...
        $challengeLogForBadge = DB::table('challenge_logs')->select('challenge_logs.*')
         //->join('user_challenges', "challenge_logs.participation_id", '=', 'user_challenges.challenge_id')
         ->where([['challenge_logs.user_id', '=', $user_id],['challenge_logs.participation_id', '=', 14]])
         ->groupBy('challenge_logs.created_at')->groupBy('challenge_logs.activity_id')
         ->get();
        $challengeLogForBadge = json_encode($challengeLogForBadge);
        $challengeLogForBadge = json_decode($challengeLogForBadge, true);
            //echo '<pre>';print_r($challengeLog); die();
        $coveredDistanceForBadge = 0;
        if($challengeLogForBadge){
            foreach($challengeLogForBadge as $key => $val){
                
                    $coveredDistanceForBadge = $coveredDistanceForBadge + $val['distance_travelled'];
                
            }

        }

        $coveredDistanceForBadge = $coveredDistanceForBadge - $distance;
                ...
                if($badge_type == 'distance'){ 
                    ...
                    if($badg){
                        ...
                        if(!$specific_date_checkbox){
                            if($coveredDistanceForBadge < $condition_limit){
        ...
        /*$user_badges = user_badges::where([['challenge_id', '=', $challenge_id], ['badge_type', '=', 'challenge_milestone']])->get();
...
//print_r($user_badges); die();
	...
	public function checkChallengeMilestone($challenge_id, $distance, $user_id, $start_date_time){
	    ...
	    */
	    ...
	    $total_distance = challenge_meta($challenge_id, 'total_distance');
	    $category = challenge_meta($challenge_id, 'category');
	    ...
	    if ($category == 'individual') {
		$challengeLog = DB::table('challenge_logs')->select('challenge_logs.*')
		    ->join('user_challenges', "challenge_logs.participation_id", '=',
		        'user_challenges.challenge_id'
		    )
		    ->where([['user_challenges.user_id', '=', $user_id], ['user_challenges.challenge_id', '=', $challenge_id
		    ]])
		    ->sum('challenge_logs.distance_travelled');

		$coveredDistance = $challengeLog;
	    } else {
		$challengeLog = DB::table('challenge_logs')->select('challenge_logs.*')
		->join('user_challenges', "challenge_logs.participation_id", '=', 'user_challenges.challenge_id')
    ...
    public function getTotalDistanceCovered($challenge_id, $distance, $total_distance,$user_id, $coveredDistance, $category, $start_date_time){    
        ...
        /*print_r($User_challenges_check);

        echo $category; echo $coveredDistance; die();*/
        ...
        //$specific_date = Carbon::parse($specific_date, 'UTC')->setTimezone(admin_timezone())->format('Y-m-d');
        $start_date_timeCM = Carbon::parse($start_date_time)->format('Y-m-d'); //die();

        $milestone = 0;
        $milestoneOnSpecificDate = 0;

        if($Challenge_milestones){ //echo '<pre>'; print_r($Challenge_milestones); echo '</pre>'; // die();
                ...
                if($v){
                     ...
                     if($orgDate){
                        ...
                        if($stat){
                           ...
                           //$newDate = \Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $orgDate);
                           $newDate = Carbon::parse($orgDate, 'UTC')->setTimezone(admin_timezone())->format('Y-m-d');
                        
                           //$newDate = date("m/d/Y H:m", strtotime($newDate));
                        }
                     }
                     if($v['id'] == 48){
                        echo $specific_date_checkbox .'&&'. $newDate.'=='. $start_date_timeCM; //die();
                         //echo '<br>';

                     }

                      
                     if($specific_date_checkbox && $newDate == $start_date_timeCM){
                        $milestoneOnSpecificDate = $v['milestone_distance'];
                        $completePercentageAtSpecificDate = $this->challengeMilestoneCompleteAtSpecificDateBadgeAssignOnLogAdd($challenge_id, $user_id, $milestoneOnSpecificDate, $newDate, $timezone, $distance, $category);
                        ...
                        if($completePercentageAtSpecificDate >= 100){
                            if($category == 'individual'){
                                ...
                                $uch = user_challenges::where('challenge_id', $challenge_id)->get();
                                $uch = json_encode($uch);
                                $uch = json_decode($uch, true);
                                if($uch){
                                    foreach($uch as $keya => $valuea){
                                        $this->assignBadgeForChallengeMilestonese($challenge_id, $v['id'], $valuea['user_id'], "challenge_milestone", $timezone);
                                    }
                                }
                            }
                            
                        }
                     }elseif(!$specific_date_checkbox){
                        $milestone = $milestone + $v['milestone_distance'];
                        $completePercentage = $this->challengeMilestoneCompleteBadgeAssignOnLogAdd($challenge_id, $user_id, $milestone, $distance, $category);
...
//echo $coveredDistance .'=='. $total_distance; 
                  ...
                  if($val['badge_type'] == 'member_since'){
                   ...
                   // $totalDistancreTravel = array_sum($totalDistancreTravelArr);
                    ...
                    $specific_date = $val['specific_date'];
                     $specific_date = Carbon::parse($specific_date, 'UTC')->setTimezone(admin_timezone())->format('Y-m-d');
                     $start_date_time = Carbon::parse($start_date_time)->format('Y-m-d'); //die();
                    ...
                    if($category == 'individual'){//echo $category; echo $challenge['price_type']; die();
                            ...
                            if($totalDistancreTravel >= $total_distance){
                                ...
                                if($challenge['price_type'] != 'default'){
                                    ...
                                    if($ucStatus != 2){
                                        ...
                                        //Mail::to($userName)->send(new ChallengeCompletionUpdateIndividual($userName->name, $challenge['name'], $total_distance, $challenge['event_start_date'], $challenge['event_end_date']));
                                    }
                        ...
                        if($challenge['price_type'] != 'default'){
                            ...
                            if($totalDistancreTravelAccu >= $total_distance){
                                ...
                                //echo $totalDistancreTravelAccu .'>='. $total_distance; die($totalDistancreTravelAccu >= $total_distance);
                       ...
                       //echo '</br>';
                        ...
                        //if($category == 'individual'){
                            $challengeLogForDate = DB::table('challenge_logs')->select('challenge_logs.*')
                             //->join('user_challenges', "challenge_logs.participation_id", '=', 'user_challenges.challenge_id')
                             ->where([['challenge_logs.user_id', '=', $user_id], ['participation_id', '=', $challenge_id]])
                             ->whereBetween('challenge_logs.startDateTime', array($start_date, $end_date))
                             ->groupBy('challenge_logs.created_at')->groupBy('challenge_logs.activity_id')
                             ->get();
                         /*}else{
    ...
    echo $coveredDistanceForDate;
    echo $condition_limit;
    echo $coveredDistance;
    die;
    */
