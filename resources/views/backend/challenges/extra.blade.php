<div class="card card-primary card-outline">
				...
				<!--- Tab for adding Venue------------------->
							...
							<form action="{{route('addlocation')}}" method="POST" enctype="multipart/form-data">
								...
								<div class="form-group">
									...
									@if (@$challenge_info['kml_file'])
									    <?php $kmlFile = asset(@$challenge_info['kml_file']); 
									    $kmlFile = str_replace("app/public","storage",$kmlFile) ?>
											<a href="{{ $kmlFile }}" target="_blank" download>view/download</a>
											<!--<div id="map"></div>-->
											<style>
												#map {
													height: 360px;
								...
								<div class="form-group">
									<label>KML Type {{@$challenge_info["kml_type"]}}</label>
									<label for="path" style="display: flex;"> <input type="radio" id="path" name="kml_type" class="form-control" value="path" style="width: 7%;" <?php echo @$challenge_info["kml_type"] == 'path' ? 'checked="checked"' : ''; ?>> Path</label><br>
									
									<label for="line" style="display: flex;"><input type="radio" id="line" name="kml_type" class="form-control" value="line" style="width: 7%;" <?php echo @$challenge_info["kml_type"] == 'line' ? 'checked="checked"' : ''; ?>> Line</label><br>
								</div>
								...
								<div class="form-group">
									...
									<input type="number" min="0" step="0.01" name="total_distance" class="form-control" placeholder="Distance in miles" value="{{@$challenge_info['total_distance']}}">
								</div>
                  ...
                  <div class="tab-pane fade" id="vert-tabs-service" role="tabpanel" aria-labelledby="vert-tabs-profile-tab">
						  ...
						  <div class="card-body">
								...
								<form action="{{route('addmilestone')}}" method="POST" enctype="multipart/form-data">
									...
									<div class="form-group distance_milestone">
										...
										<input type="number" min="0" step="0.01" name="milestone_distance" class="form-control" placeholder="Distance" value="">
									</div>
                    ...
                    <div class="input-group date specific_date" id="specific_date" data-target-input="nearest" style="display:none;">
									...
									<div class="form-group">
				            <label for="exampleInputEmail1">Milestone Information</label>
				            <textarea class="form-control" name="milestone_info"></textarea>
				          </div>
    ...
    <div class="modal fade" id="modal-edit-milestone">
        <div class="modal-dialog modal-xl">
          <div class="modal-content">
            ...
            <div class="modal-body">
              <section class="content">
		                    ...
		                    <div class="input-group date specific_date" id="specific_date_edit" data-target-input="nearest" style="display:none;">
											...
											<div class="form-group">
						            <label for="milestone_info">Milestone Information</label>
						            <textarea class="form-control" name="milestone_info" id="milestone_info"></textarea>
						          </div>
            ...
            <div class="modal-footer justify-content-between">
              <button type="button" class="btn btn-default close-Mile-Model" data-dismiss="modal">Close</button>
            </div>
          </div>
          <!-- /.modal-content -->
        </div>
  ...
  function valueChanged()
	...
	function edit_me(id, milestone_name, milestone_type, start_date, end_date, specific_date, milestone_distance, specific_date_checkbox, milestone_pic, milestone_info, el){
		...
		$('textarea[name=milestone_info]').val(milestone_info);
		if(specific_date_checkbox){
			$('input[name=specific_date_checkbox]').attr('checked','checked');
			$('.specific_date').css('display', '');
		}
